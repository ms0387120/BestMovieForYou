...
def crawl_upcoming_movies_id():
    ...
    url_stem = 'https://api.themoviedb.org/3/movie/upcoming?'
    url_var = '&'.join(
...
movies = crawl_upcoming_movies_id()

# Save upcoming movies
data_file = os.path.join(data_folder, 'upcoming_movies.json')
with open(data_file, 'w') as stream:
    json.dump(movies, stream)

# Load upcoming movies and transform to dictionary
data_file = os.path.join(data_folder, 'upcoming_movies.json')
...
# Save upcoming movies details
data_file = os.path.join(data_folder, 'upcoming_movies_details.json')
with open(data_file, 'w') as stream:
    json.dump(movies_detail, stream)
    
# Load upcoming movies details and transform to dictionary
data_file = os.path.join(data_folder, 'upcoming_movies_details.json')
with open(data_file, 'r') as stream:
