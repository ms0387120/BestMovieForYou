...
def search_similar_movies(request):
    ...
    ref_synopsis = crawl_imdb_synopsis(ref_imdb_id)
    ref_synopsis_parsed = parse_corpus(ref_synopsis)
    
    from multiprocessing import Pool
    N = 10 # number of threads
    
    
    url = 'https://best-movie-for-you.firebaseio.com/{}_synopsis_parsed.json?print=pretty'.format(movies_pool)
    r = requests.get(url)
    db_movies_synopsis_parsed = json.loads(r.content.decode("utf-8") )
    ...
    similarity_scores = p.map(gen_all_similarity_scores, list(itertools.zip_longest([],
                              db_movies_synopsis_parsed.items(), fillvalue=ref_synopsis_parsed)))
    similarity_scores = sorted(similarity_scores, key=lambda x: x[1], reverse=True)
